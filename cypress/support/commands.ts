...
Cypress.Commands.add(
  ...
  (selector: string, timeout: number = 10000) => {
    /**
...
Cypress.Commands.add(
  ...
  (
    method: string,
    url: string | RegExp,
    assertFn: (interception: any) => void
  ) => {
    /**
...
Cypress.Commands.add(
  ...
  (method: string, url: string, threshold: number) => {
    /**
      ...
      if (src) {
        cy.window().then((win) => {
          ...
          const entry = entries.find(
            (e) => (e as PerformanceResourceTiming).name === src
          );
...
Cypress.Commands.add(
  "isNotCovered",
  (selector: string, blockingSelector?: string) => {
    /**
    ...
    cy.get(selector).then(($el) => {
      ...
      if (blockingSelector) {
        ...
        cy.document().then((doc) => {
          const elAtPoint = doc.elementFromPoint(
            elRect.left + 1,
            elRect.top + 1
          );
...
Cypress.Commands.add(
  "hasPointerEvents",
  (selector: string, value: string = "auto") => {
    /**
...
Cypress.Commands.add(
  "hasStyle",
  (selector: string, style: Partial<CSSStyleDeclaration>) => {
    /**
...
Cypress.Commands.add(
  "mockApi",
  (method: Method, url: string, mockData: string | object) => {
    /**
     ...
     */
    const response =
      typeof mockData === "string" ? { fixture: mockData } : mockData;
...
Cypress.Commands.add(
  "waitForUrl",
  (url: string | RegExp, timeout: number = 10000) => {
    /**
